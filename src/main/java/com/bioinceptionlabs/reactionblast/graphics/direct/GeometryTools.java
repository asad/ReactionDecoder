 ...
 */
package com.bioinceptionlabs.reactionblast.graphics.direct;
 ...
 * All methods in this class change the coordinates of the atoms. Use
 * GeometryTools if you use an external set of coordinates (e. g.
...
public class GeometryTools {
     ...
     * @see
     * GeometryTools#get3DCoordinateCoverage(org.openscience.cdk.interfaces.IAtomContainer)
     */
    ...
    /**
     * Translates the given molecule by the given Vector. See comment for
     ...
     * dimension. See comment for center(IAtomContainer atomCon, Dimension
     * areaDim, HashMap renderingCoordinates) for details on coordinate sets
     *
    ...
    /**
     * Multiplies all the coordinates of the atoms of the given molecule with
     * the scalefactor. See comment for center(IAtomContainer atomCon, Dimension
     * areaDim, HashMap renderingCoordinates) for details on coordinate sets
     *
     * @param atomCon The molecule to be scaled
    ...
    /**
     * Centers the molecule in the given area. See comment for
     * center(IAtomContainer atomCon, Dimension areaDim, HashMap
     * renderingCoordinates) for details on coordinate sets
     *
     ...
     *
     * The code is based on code available
     * <a href="http://astronomy.swin.edu.au/~pbourke/geometry/rotate/source.c">here</a>.
     * Positive angles are anticlockwise looking down the axis towards the
     * origin. Assume right hand coordinate system.
     *
    ...
    /**
     * Returns the java.awt.Dimension of a molecule. See comment for
     * center(IAtomContainer atomCon, Dimension areaDim, HashMap
     * renderingCoordinates) for details on coordinate sets
     *
     ...
     *   minmax[2] = maxX;
     *   minmax[3] = maxY;
     * </pre> See comment for center(IAtomContainer atomCon, Dimension areaDim,
     * HashMap renderingCoordinates) for details on coordinate sets
     *
    ...
    /**
     * Translates the geometric 2DCenter of the given AtomContainer container to
     * the specified Point2d p.
     *
     ...
     *
     * @param coords The coordinates of the two given points of the bond like
     * this [point1x, point1y, point2x, point2y]
     ...
     * array. See comment for center(IAtomContainer atomCon, Dimension areaDim,
     * HashMap renderingCoordinates) for details on coordinate sets
     *
    ...
    /**
     * Returns the atom of the given molecule that is closest to the given
     * coordinates and is not the atom. See comment for center(IAtomContainer
     * atomCon, Dimension areaDim, HashMap renderingCoordinates) for details on
     * coordinate sets
     *
     * @param xPosition The x coordinate
     * @param yPosition The y coordinate
    ...
    /**
     * Determines the scale factor for displaying a structure loaded from disk
     * in a frame. An average of all bond length values is produced and a scale
     ...
     * @return The ScaleFactor with which the AtomContainer must be scaled to
     * have the target bond length
     */
     ...
     * Atom's with no coordinates are disregarded. See comment for
     * center(IAtomContainer atomCon, Dimension areaDim, HashMap
     * renderingCoordinates) for details on coordinate sets
     *
    ...
    /**
     * Determines if all this {@link IAtomContainer}'s atoms contain 2D
     * coordinates. If any atom is null or has unset 2D coordinates this method
     * will return false.
     *
    ...
    /**
     * Determines the coverage of this {@link IAtomContainer}'s 2D coordinates.
     * If all atoms are non-null and have 2D coordinates this method will return
     * {@link CoordinateCoverage#FULL}. If one or more atoms does have 2D
     * coordinates and any others atoms are null or are missing 2D coordinates
     * this method will return {@link CoordinateCoverage#PARTIAL}. If all atoms
     * are null or are all missing 2D coordinates this method will return
    ...
    /**
     * Determines if this Atom contains 2D coordinates. See comment for
     * center(IAtomContainer atomCon, Dimension areaDim, HashMap
     * renderingCoordinates) for details on coordinate sets
     *
    ...
    /**
     * Determines if this Bond contains 2D coordinates. See comment for
     * center(IAtomContainer atomCon, Dimension areaDim, HashMap
     * renderingCoordinates) for details on coordinate sets
     *
     * @param bond Description of the Parameter
    ...
    /**
     * Determines if all this {@link IAtomContainer}'s atoms contain 3D
     * coordinates. If any atom is null or has unset 3D coordinates this method
     * will return false. If the provided container is null false is returned.
     *
     * @param container the atom container to examine
     *
    ...
    /**
     * Returns a Map with the AtomNumbers, the first number corresponds to the
     * first (or the largest AtomContainer) atomcontainer.It is recommend to
     * sort the atomContainer due to their number of atoms before calling this
     * function. The molecules needs to be aligned before! (coordinates are
     * needed)
     *
    ...
    public static Map<Integer, Integer> mapAtomsOfAlignedStructures(IAtomContainer firstAtomContainer,
        ...
        //DEBUGG
        /*
         * double tmp=0; for(int i=0;i<firstAtomContainer.getAtomCount();i++){
         * //
         * logger.debug(i+" "+firstAtomContainer.getAtomAt(i).getSymbol()+"\t");
         * for (int j=0;j<secondAtomContainer.getAtomCount();j++){
         * tmp=Math.floor(distanceMatrix[i][j]*10); //logger.debug(tmp/10+"\t");
         * } }
         */
     ...
     * @param secondAtomContainer the second aligned AtomContainer
     * @param mappedAtoms Map: a Map of the mapped atoms
     * @param Coords3d boolean: true if molecules has 3D coords, false if
     * molecules has 2D coords
     * @return double: the value of the RMSD
     ...
     * @param secondAtomContainer the second aligned AtomContainer
     * @param mappedAtoms Map: a Map of the mapped atoms
     * @param hetAtomOnly boolean: true if only hetero atoms should be
     * considered
     * @param Coords3d boolean: true if molecules has 3D coords, false if
     * molecules has 2D coords
     * @return double: the value of the RMSD
     ...
     * @param bounds the {@link Rectangle2D} of the {@link IAtomContainer} to
     * shift
     * @param last the bounds that is used as reference
     * @param gap the gap between the two {@link Rectangle2D}s
     * @return the {@link Rectangle2D} of the {@link IAtomContainer} after the
     * shift
     */
     ...
     * @return Boolean indication that 3D coordinates are available for all
     * atoms.
     */
    ...
    /**
     * Shift the containers in a reaction vertically upwards to not overlap with
     * the reference Rectangle2D. The shift is such that the given gap is
     * realized, but only if the reactions are actually overlapping.
     *
