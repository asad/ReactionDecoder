 ...
 * betwwen two edges of the source graphs G1 and G2 that are compared. Two edges
 * may be associated if they have at least one common feature. The association
 * is defined outside this class. The node keeps tracks of the ID of the mapped
 * edges (in an CDKRMap), of its neighbours in the RGraph it belongs to and of
 * the set of incompatible nodes (nodes that may not be along with this node in
 * the same solution)
 *
 * @author Stephane Werner from IXELIS mail@ixelis.net 2002-07-17 smsd
 *
 */
public class CDKRNode {
     ...
     */
    synchronized void setRMap(CDKRMap rMap) {
        this.setrMap(rMap);
    }
