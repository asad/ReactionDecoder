 ...
 * </pre></blockquote>
 *
 * The primary function is to provide an iterable of matches - each match is a
 * permutation (mapping) of the query graph indices (atom indices).
 *
 * <blockquote><pre>{@code
 * for (int[] p : mappings) {
...
public final class Mappings implements Iterable<int[]> {
     ...
     * @see Pattern
     */
    public Mappings(IAtomContainer query, IAtomContainer target, Iterable<int[]> iterable) {
        this.query = query;
        this.target = target;
    ...
    /**
     * Obtain the chem objects (atoms and bonds) that have 'hit' in the target
     * molecule.
     *
     * <blockquote><pre>
    ...
    /**
     * Obtain the mapped substructures (atoms/bonds) of the target compound. The
     * atoms and bonds are the same as in the target molecule but there may be
     * less of them.
     *
     * <blockquote><pre>
     ...
     * manually.
     *
     * The method is simply invokes
     * <pre>mappings.uniqueAtoms().count()</pre>.
     *
     * @return number of matches
     */
