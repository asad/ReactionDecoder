...
public interface IAtomMapping {
    ...
    /**
     * Returns summation energy score of the disorder if the MCS is removed from
     * the target and query graph. Amongst the solutions, a solution with lowest
     * energy score is preferred.
     *
    ...
    /**
     * Returns number of fragment generated in the solution space, if the MCS is
     * removed from the target and query graph. Amongst the solutions, a
     * solution with lowest fragment size is preferred.
     *
     * @param Key Index of the mapping solution
    ...
    /**
     * Returns a number which denotes the quality of the mcs. A solution with
     * highest stereo score is preferred over other scores.
     *
     * @param Key Index of the mapping solution
     * @return true if no stereo mismatch occurs else false if stereo mismatch
     * occurs
     */
    ...
    /**
     * Returns Tanimoto similarity between query and target molecules (Score is
    ...
    /**
     *
     * Returns true if mols have different stereo chemistry else false if no
     * stereo mismatch.
     *
     * @return true if mols have different stereo chemistry else false if no
     * stereo mismatch. true if stereo mismatch occurs else true if stereo
     * mismatch occurs.
     */
    public abstract boolean isStereoMisMatch();

    /**
     * Returns total mapping count between query and target molecules Each map
     * in the list has atom-atom equivalence of the mappings between query and
     * target molecule i.e. map.getKey() for the query and map.getValue() for
     * the target molecule.
     *
     * @return All possible MCS atom Mappings
     */
