 ...
 * (MCS) but could easily be extended to handle such cases.
 * <p/>
 * The class implements the Guava predicate and can be used easily filter the
 * mappings.
 *
 * <blockquote><pre>{@code
 ...
 */
final class StereoMatch implements Predicate<Map<Integer, Integer>> {
    ...
    /**
     * Create a predicate for checking mappings between a provided {@code query}
     * and {@code target}.
     *
     ...
     */
    @Override
    public boolean apply(final Map<Integer, Integer> mapping) {
     ...
     */
    public boolean checkTetrahedral(int u, Map<Integer, Integer> mapping) {
        int v = mapping.get(u);
     ...
     */
    private int[] map(int u, int v, int[] us, Map<Integer, Integer> mapping) {
        ...
        if (query.getAtom(u).getImplicitHydrogenCount() == 1 && target.getAtom(v).getImplicitHydrogenCount() == 0) {
            ...
            mapping.put(u, targetMap.get(explicitHydrogen));
        }

        for (int i = 0; i < us.length; i++) {
            us[i] = mapping.get(us[i]);
        }

        mapping.put(u, v); // remove temporary mapping to hydrogen
        return us;
    }
     ...
     */
    public boolean checkGeometric(int u1, int u2, Map<Integer, Integer> mapping) {

        int v1 = mapping.get(u1);
        int v2 = mapping.get(u2);
        ...
        if (mapping.get(uLeft) != vLeft) {
            p *= -1;
        }
        if (mapping.get(uRight) != vRight) {
            p *= -1;
        }
