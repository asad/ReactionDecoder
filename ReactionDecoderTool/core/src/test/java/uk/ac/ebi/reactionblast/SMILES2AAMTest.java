...
public class SMILES2AAMTest extends TestUtility {
    ...
    public void testMain() throws URISyntaxException {
        ...
        File dir = new File(path, RXN_DIR);
        try {
            out.println("Root Directory: " + path);
            out.println("RXN File Directory: " + dir.getCanonicalPath());
        ...
        if (dir.isDirectory()) {
            ...
            for (File file : files) {
                ...
                try {
                    readReaction = readReactionFile(file.getName().split("\\.")[0], dir.getCanonicalPath() + separator, true, false);
